<template>
  <div id="mob-quests" v-if="loaded">
    ...
    <div v-if="!quests.length">No quest uses this item.</div>
  </div>
</template>
...
@Component
export default class ItemTemplateQuests extends Mixins(KeepAliveFetch) {
  loaded: boolean = false;

  get quests() {
    return this.$store.state.builder.worlds.item_template_quests;
  }
  ...
  async fetch() {
    this.$store.dispatch("builder/worlds/item_template_quests_fetch", {
      world_id: this.$route.params.world_id,
