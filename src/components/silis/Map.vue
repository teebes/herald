  ...
  <div class="map-view">
    ...
    <div class="planes" v-if="show_planes">
      <div class="arrow-up-wrapper">
        <img v-if="center.up" src="@/assets/ui/arrow-70.svg" class="plane-arrow plane-arrow-up" @click="onClickUp" />
      </div>
      <img src="@/assets/ui/plane-full-70.svg" class="plane" />
      <img src="@/assets/ui/plane-hollow-70.svg" class="plane" />
      <img src="@/assets/ui/plane-full-70.svg" class="plane" />
      <div class="arrow-down-wrapper">
        <img v-if="center.down" src="@/assets/ui/arrow-70.svg" class="plane-arrow plane-arrow-down"
          @click="onClickDown" />
      </div>
    </div>
  </div>
...
import { get_map_width } from "@/core/map_silis";
import MapRenderer from "@/core/map_silis";
...
const center = computed(() => props.map[props.center_id]);
...
const onClick = (event) => {
  ...
  if (room) {
    let room_id = room.id;
    // if (room.id.split('-exit-').length > 1) {
    //   room_id = room.id.split('-exit-')[0];
    // }
    const clickedRoom = props.map[room_id];
    emit('clickRoom', clickedRoom);
  }
};

const onClickUp = () => {
  const room = props.map[props.center_id];
  const up = room.up;
  if (up) {
    emit('clickRoom', props.map[up]);
  }
};
...
const get_rooms_around = (room, radius) => {
  ...
  const index = {};
  for (const room_id in props.map) {
    const room = props.map[room_id];
      ...
      for (let z = room.z - 1; z <= room.z + 1; z += 1) {
        ...
        if (roomAtCoords) {
          ...
          if (
            ...
            !rooms_filter_index.value[roomAtCoords.id]
          ) {
            continue;
          }

          // Add to the returned rooms
          roomsAround[roomAtCoords.id] = roomAtCoords;
        }
      }
    }
  }
...
const renderMap = () => {
  console.log('renderMap', props.center_id);
  ...
  console.log('rooms around: ', rooms);
  ...
  for (const room_id in rooms) {
    ...
    for (let direction of directions) {
      ...
      if (exitId && !rooms[exitId]) {
        ...
        for (let dir of directions) exitRoom[dir] = null;
        //exitRoom[REVERSE_DIRECTIONS[direction]] = room.id;
        connectedExits[exitRoom.id] = exitRoom;
      }
    }
  }
  ...
  console.log('calling showView', props.center_id);
  mapRenderer.value.showView(props.center_id);
...
watch(() => props.center_id, () => {
  if (props.center_id) renderMap();
});
