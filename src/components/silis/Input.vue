<template>
  <div class="cmd-sent">
    {{ store.state.silis.cmd_sent || '&nbsp;' }}
  </div>
  <div class="cmd-response">
    <span v-if="store.state.silis.cmd_response">
      {{ store.state.silis.cmd_response }}
    </span>
    <span v-else-if="store.state.silis.cmd_sent">
      ...
    </span>
    <span v-else>&nbsp;</span>
  </div>
...
const onSubmit = () => {
  ...
  if (user_input) {
    store.dispatch("silis/cmd", user_input);
  }
...
const onKeyDown = (event: KeyboardEvent) => {
  ...
  if (direction) {
    ...
    store.dispatch("silis/cmd", direction);
  }
};
