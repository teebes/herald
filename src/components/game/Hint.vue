...
export default class extends Vue {
  get hintText() {
    const parts = this.$store.state.game.hint.split(":");
  ...
  get hintID() {
    return this.$store.state.game.hint.split(":")[0];
  }

  get isInverted() {
    if (this.hintID === "6" || this.hintID === "8" || this.hintID === "9") {
      if (this.$store.state.game.is_mobile) return true;
    }
    return false;
  }

  get isSouthSpecial() {
    if (this.hintID === "8" && this.$store.state.game.is_mobile) {
      return true;
    }
    return false;
  }
