...
const actions = {
  ...
  approve_review: async ({ commit, dispatch }, data) => {
    const resp = await axios.post(`/builder/worlds/${data.world.id}/reviews/${data.id}/resolve/`, {
      'status': 'approved',
      'text': data.text,
    });
    dispatch('load_endpoints');
  ...
  reject_review: async ({ commit, dispatch }, data) => {
    const resp = await axios.post(`/builder/worlds/${data.world.id}/reviews/${data.id}/resolve/`, {
      'status': 'reviewed',
      'text': data.text,
    });
